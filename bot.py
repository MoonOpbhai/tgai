...
# ✅ Correct model names (PyPI docs se verified)
MODELS = {
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"/help - Help dekho\n\n"
        f"Kuch bhi type karo aur main jawab dunga!"
    )
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Bot conversation yaad rakhta hai\n"
...
async def model_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"Model set: {label}\n\nHistory clear ho gayi. Ab baat karo!")


async def clear_history(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    state = get_user_state(update.effective_user.id)
    state["history"] = []
    await update.message.reply_text("Chat history clear ho gayi! Fresh start karo.")
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # ✅ Correct signature: prompt pehle, model keyword argument
        full_response = ""
        for chunk in client.stream_chat(full_prompt, model=state["model"]):
            full_response += chunk
...
def main():
    ...
    if WEBHOOK_URL:
        ...
        logger.info("Polling mode (local test)...")
